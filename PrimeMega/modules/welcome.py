...
def welcome_help(update: Update, context: CallbackContext):
    chat = update.effective_chat
    update.effective_message.reply_text(text=gs(chat.id, "WELCOME_HELP_TEXT"), parse_mode=ParseMode.MARKDOWN)
...
def new_member(update: Update, context: CallbackContext):
    ...
    if not was_member and is_member:
        ...
        if new_mem.id == bot.id and not PrimeMega.ALLOW_CHATS:
            with suppress(BadRequest):
                update.effective_chat.send_message(f"Groups are disabled for {bot.first_name}, I'm outta here.")
            ...
            # Welcome yourself
                ...
                if creator:
                    ...
                    bot.send_message(
                        ...
                        parse_mode=ParseMode.HTML)
                update.effective_message.reply_text(
                    f"Thanks for adding me! Join @{SUPPORT_CHAT} for support.", reply_to_message_id=reply)
